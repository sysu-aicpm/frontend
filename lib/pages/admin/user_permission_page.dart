...
import 'package:smart_home_app/bloc/user_permission/bloc.dart';
...
class _UserPermissionPageState extends State<UserPermissionPage> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: BlocProvider(
        create: (context) => bloc..add(LoadUserPermission(widget.userId)),
        child: BlocBuilder<UserPermissionBloc, UserPermissionState>(
          builder: (context, state) {
            if (state is UserPermissionLoading) {
              return const Center(
                child: CircularProgressIndicator(),
              );
            }

            if (state is UserPermissionError) {
              return Center(
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton.icon(
                      onPressed: () {
                        context
                            .read<UserPermissionBloc>()
                            .add(LoadUserPermission(widget.userId));
                      },
            ...
            if (state is UserPermissionLoaded) {
              return TabBarView(
                controller: _tabController,
                children: [
  ...
  Widget _buildDevicePermissionsTab(UserPermissionLoaded state, UserPermissionBloc bloc) {
  ...
  Widget _buildDeviceGroupPermissionsTab(UserPermissionLoaded state, UserPermissionBloc bloc) {
  ...
  Widget _buildDeviceCard(Device device, DevicePermission permission, UserPermissionBloc bloc) {
    return Container(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            _buildPermissionSelector(
              permission.level,
              (newLevel) {
                bloc.add(UpdateUserPermissionOnDevice(device.id, newLevel));
              },
            ),
          ],
        ),
        ...
        subtitle: Column(
          ...
          children: [
            ...
            _buildPermissionSelector(
              ...
              (newLevel) {
                bloc.add(
                  UpdateUserPermissionOnDeviceGroup(
                    deviceGroup.id,
                    newLevel,
                  ),
                );
